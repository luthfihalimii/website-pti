...
@section('content')
  <section class="rounded-[30px] border border-slate-200/80 bg-white p-6 shadow-sm">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-blue-700">{{ __('Rekrutmen') }}</p>
        <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Lowongan Pekerjaan</h1>
        <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-600">Kelola daftar posisi yang ditampilkan pada halaman lowongan publik.</p>
      </div>
      <div class="flex items-center gap-3">
        <div class="rounded-2xl bg-slate-950 px-5 py-4 text-white shadow-lg shadow-slate-950/10">
          <p class="text-[11px] uppercase tracking-[0.28em] text-blue-300">Total Lowongan</p>
          <p class="mt-2 text-3xl font-semibold">{{ $vacancies->total() }}</p>
        </div>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-[760px] divide-y divide-slate-200 text-sm">
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          @empty
            <tr>
              <td colspan="5" class="px-5 py-12 text-center text-slate-500">Belum ada lowongan pekerjaan.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
